...
if __name__ == "__main__":
    ...
    for shots in shot_sg:
        ...
        for n_type in nts:
            ...
            out_path = "./check_points/noise_type_1/model.pth"

            model.load_state_dict(torch.load(out_path))

            [_, _, X_test, y_test] = Data_Preparation(n_type) #X加噪

            # X_test = torch.FloatTensor(X_test)
            # X_test = X_test.permute(0,2,1)

            # ytest存盘
            # a = y_test.reshape(-1,y_test.shape[2])[:, 0]
            # # 创建一个 DataFrame 对象
            # df = pd.DataFrame({'Column A': a})
            # # 指定要保存的 Excel 文件路径和文件名
            # excel_file_path = './output/ytest.xlsx'
        ...
        num_rows, num_cols, _ = out_numpy.shape
        outsum = out_numpy.reshape(num_rows * num_cols)
        num_rows, num_cols, _ = noisy_numpy.shape
        noisysum = noisy_numpy.reshape(num_rows * num_cols)
        num_rows, num_cols, _ = clean_numpy.shape
        cleansum = clean_numpy.reshape(num_rows * num_cols)
        df = pd.DataFrame({'outsum': outsum, 'noisy': noisysum,'clean': cleansum})
        # 指定要保存的 Excel 文件路径和文件名
        excel_file_path = './output/sel123_data_out_noisy0.2-2.0NEW_shot'+ str(shots) + '.xlsx'
        # 将 DataFrame 写入 Excel 文件
        df.to_excel(excel_file_path, index=False)
